#!/usr/bin/env python3
"""GC Agent: Зинаида. Подключается как human, отвечает через DeepSeek."""
...
NAME = "Зинаида"
SYSTEM = "Ты Зинаида. Отвечай на русском. Ты в групповом чате."
USER_ID = "zinaida_agent"
...
def on_all(event, *args):
    ...
    # Явный callback чтобы отловить ошибку
    def cb(*args):
        if args and args[0]:
            print("  !! send error: {}".format(args[0]), flush=True)
        else:
            print("  !! send OK (no args)", flush=True)
    sio.emit("message", {"roomId": ROOM, "content": reply, "id": "zinaida_{}".format(int(time.time()*1000)),
        "timestamp": int(time.time() * 1000)}, namespace="/group-chat", callback=cb)
...
sio.connect(WEB_UI, namespaces=["/group-chat"],
    auth={"token": JWT, "source": "human", "userId": "1", "name": "Зинаида", "authUserId": 1},
