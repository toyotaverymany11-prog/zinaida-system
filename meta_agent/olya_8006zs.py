...
"""
================================================================================
 OLYA 8006zs — Super Cascade (Zinaida Style)
================================================================================
Порт: 8006
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [OLYA6zs] %(message)s")
logger = logging.getLogger("olya_8006zs")
logging.getLogger("httpx").setLevel(logging.WARNING)

app = FastAPI(title="Olya 8006zs Cascade Router")
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=httpx.Timeout(30, connect=5)) as c:
                resp = await c.post(
                    OLLAMA_URL,
                    headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"},
                    json={"model": "gemma4:31b", "messages": messages,
...
async def _generate(messages: List[dict], is_stream: bool, timeout: float,
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=httpx.Timeout(10, connect=5)) as c:
                resp = await c.post(
                    OLLAMA_URL,
                    headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"},
                    json={"model": "gemma4:31b", "messages": messages, "max_tokens": 50, "temperature": 0.5}
                )
                if resp.status_code == 200:
                    data = resp.json()
                    text = data["choices"][0]["message"]["content"]
                    logger.info("OLLAMA OK: ответил за %.1f сек", time.time() - start)
                    await _log_request("ollama/gemma4:31b", len(last_user), time.time() - start, "ok")
    ...
    # 4c: DeepSeek Pro (если бесплатные не справились — эскалация)
    if not response_text:
        model_key_ds = "pro" if model_key == "flash" else model_key
        model_id = MODELS.get(model_key_ds, MODELS["pro"])["id"]
        ...
        try:
            ds_timeout = PRO_TIMEOUT if (model_key == "pro" or model_key_ds == "pro") else FLASH_TIMEOUT
            ...
            logger.info("DEEPSEEK (%s) OK: %d символов за %.1f сек",
                        model_key_ds, len(response_text), time.time() - start)
...
async def status():
    ...
    return {
        "router": "8006zs — Super Cascade (Olya)",
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("OLYA 8006zs стартует на порту 8006")
    ...
    logger.info("=" * 60)
    uvicorn.run(app, host="127.0.0.1", port=8006, log_level="info", log_config=None)
